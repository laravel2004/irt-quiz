...
<div class="container" style="padding: 40px 20px; max-width: 1000px; margin: 0 auto;">
    <div style="margin-bottom: 32px; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 16px;">
        <a href="{{ route('participant.review', $registration->id) }}" class="btn-primary" style="background: rgba(255,255,255,0.1); color: white; border: none; padding: 10px 20px; text-decoration: none; display: inline-flex; align-items: center; gap: 8px;">
            <i class="fas fa-arrow-left"></i> Kembali ke Daftar Laporan
        </a>
        <div style="text-align: right;">
            <h4 style="font-family: 'Outfit', sans-serif; margin-bottom: 4px; color: white;">{{ $registration->examSession->name }}</h4>
    ...
    <div class="glass animate-fade-in" style="padding: 32px; margin-bottom: 24px; border-left: 6px solid {{ $participantAnswer === null ? '#94a3b8' : ($isCorrect ? '#10b981' : '#ef4444') }};">
        ...
        <div style="display: grid; gap: 12px; margin-bottom: 24px;">
            ...
            <div style="padding: 16px; border-radius: 12px; border: 1px solid {{ $borderColor }}; background: {{ $bgColor }};">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                    <span style="color: white; font-weight: 500;">{{ $optIndex + 1 }}. {{ $option }}</span>
                </div>
                <div style="display: flex; gap: 12px; align-items: center; font-size: 0.85rem;">
                    <span style="color: var(--text-secondary);">Jawaban Anda: <strong style="color: {{ $userAns ? ($statementCorrect ? '#10b981' : '#ef4444') : '#94a3b8' }};">{{ $userAns ? ucfirst($userAns) : 'Tidak dijawab' }}</strong></span>
        ...
        <div style="background: rgba(59, 130, 246, 0.1); padding: 12px 16px; border-radius: 8px; margin-bottom: 16px; font-size: 0.9rem; color: var(--accent);">
