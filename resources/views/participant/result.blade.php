...
<div class="container" style="padding: 40px 20px; max-width: 800px; margin: 0 auto;">
    ...
    <div class="glass animate-fade-in" style="padding: 40px; border-radius: 24px; text-align: center;">
        <div style="margin-bottom: 40px;">
            ...
            <p style="color: #475569;">{{ $registration->examSession->name }}</p>
        </div>
        ...
        <div class="stats-grid" style="background: #f8fafc; border-radius: 16px; padding: 24px; display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px;">
            <div>
            ...
            <div>
                ...
                <div style="font-size: 1.25rem; font-weight: 700; color: #0f172a;">{{ $registration->result->total_blank }}</div>
            </div>
        </div>

        <div style="margin-top: 48px; padding-top: 32px; border-top: 1px solid var(--glass-border); color: #475569; font-size: 0.85rem;">
            ...
            <div class="btn-group" style="display: flex; gap: 12px; justify-content: center; margin-top: 24px;">
                @if($registration->examSession->discussion_pdf)
                    ...
                    @else
                    <button onclick="Swal.fire({icon: 'lock', title: 'Akses Terkunci', text: 'File Pembahasan khusus untuk peserta Premium. Silakan hubungi Admin untuk upgrade akses Anda.', background: '#ffffff', color: '#0f172a', confirmButtonColor: '#eab308'});" class="btn-primary" style="height: 40px; font-size: 0.85rem; background: rgba(234, 179, 8, 0.1); color: #eab308; border: 1px solid #eab308; cursor: pointer;">
                        <i class="fas fa-lock"></i> Download Pembahasan (PDF)
                    </button>
                    @endif
                @endif
        ...
        <div style="margin-top: 40px; border-top: 1px solid var(--glass-border); padding-top: 40px; text-align: left;">
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 24px;">
                ...
                @if($registration->privilege === 'premium')
                <span class="badge" style="background: rgba(234, 179, 8, 0.1); color: #eab308; font-size: 0.75rem;"><i class="fas fa-crown"></i> Akses Premium</span>
                @endif
            </div>
